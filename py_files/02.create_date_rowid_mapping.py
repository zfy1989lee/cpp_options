...
for j in range(0,30):
   ...
   for m_date in results:
      print(m_date[0])
      query = r"SELECT MIN(rowid), MAX(rowid) from %s_new1 where quotedate='%s' and rowid>%s and rowid<%s" % (_ccypair,str(m_date[0]),str(prevrowid),str(prevrowid+10e6))
      print(query)
      cur.execute(query)
      ...
      cur.execute(query)
      print(j, min_rowid[0],min_rowid[1],(min_rowid[1]-min_rowid[0])/10e6)
